    ...
    .then((docSnapshot) => {
      if (docSnapshot.exists) {
        ...
        if (data.rollNumbers && Array.isArray(data.rollNumbers)) {
          ...
          const numberOfPresentees = rollNumbers.length; // Calculate the count of roll numbers

          console.log('Fetched roll numbers:', rollNumbers);
          console.log('Number of presentees:', numberOfPresentees);

          res.status(200).json({ date, rollNumbers, numberOfPresentees });
        } else {
          console.log('No roll numbers found for date:', date);
          res.status(200).json({ date, rollNumbers: [], numberOfPresentees: 0 }); // Empty array and zero count if no roll numbers
        }
      } else {
